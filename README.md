...
> 一个已发表的结核 signature 在三个队列都能区分 progressor 与 non-progressor（AUC 0.68 至 0.87）。某个队列失败的只是它的*时间梯度*，而这跟着采样窗口走，不是批次效应。四种校正方法（直到深度生成模型）都在改善批次混合但是无法解决问题。要点：在动用更大的模型之前，先审计研究设计。
